...
from app.schemas.marks_schema import GenerateSingleStudentsSingleSemesterResultResponseSchema, MarksCreateSchema, MarksUpdateSchema, SemesterWiseAllSubjectsMarksWithPopulatedDataResponseSchema
...
async def generate_single_students_single_semester_result(
    ...
    try:
        ...
        logger.critical(f"Generate results unexpected Error: {e}")
        # attach audit payload
        if request:
            request.state.audit_payload = {
                "raw_error": str(e),
